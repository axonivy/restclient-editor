...
const tsOut = path.resolve('./src/data/restclient.ts');
var schemaUri =
  'https://jenkins.ivyteam.io/job/core_json-schema/job/master/lastSuccessfulBuild/artifact/build/schema/target/editor-ts/14.0/restclients-tsgen.json';
...
if (schemaUri.startsWith('http')) {
  loadJson(schemaUri)
    .then(schema => tsGen.compile(schema, 'restclients'))
    .then(ts => writeSrc(ts));
} else {
