...
export const useKnownHotkeys = () => {
  ...
  const addRestClient = useMemo<KnownHotkey>(() => {
    const hotkey = 'A';
    return { hotkey, label: t('hotkey.addRestClient', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);

  const deleteRestClient = useMemo<KnownHotkey>(() => {
    const hotkey = 'Delete';
    return { hotkey, label: t('hotkey.deleteRestClient', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);
  ...
  return { undo, redo, openHelp, addRestClient, deleteRestClient, focusToolbar, focusMain, focusInscription };
};
