...
export default defineConfig({
  ...
  webServer: {
    command: `pnpm run --filter @axonivy/restclient-editor-standalone ${process.env.CI ? 'serve' : 'dev'}`,
  ...
  projects: [
    ...
    { name: 'modify', use: { ...devices['Desktop Chrome'] }, workers: 1, testDir: './tests/modify' },
